...
#define  KEY  AES_KEY
#define AES_KEY  @"abcdefgabcdefg12"
...
+ (NSData*)convertHexStrToData:(NSString*)str {
    ...
    NSRange range;
    if ([str length] %2==0) { range = NSMakeRange(0,2);
    } else {
...
+ (NSString*)convertDataToHexStr:(NSData*)data
{
    ...
    [data enumerateByteRangesUsingBlock:^(const void*bytes,NSRange byteRange,BOOL*stop) {
        ...
        for (NSInteger i =0; i < byteRange.length; i++) { NSString *hexStr = [NSString stringWithFormat:@"%x", (dataBytes[i]) & 0xff];
            if ([hexStr length] ==2) {
